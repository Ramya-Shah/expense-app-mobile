...
export const AddTransactionScreen: React.FC<Props> = ({ navigation }) => {
  const { state, addTransactionAsync, addCategoryAsync } = useContext(AppContext);
  const [type, setType] = useState<'expense' | 'income'>('expense');
  const [amount, setAmount] = useState('');
  const [description, setDescription] = useState('');
  const [category, setCategory] = useState(state.categories.length > 0 ? state.categories[0].name : '');
